...
def plot_leaf_mass_vs_height_species(df):
    ...
    odir = "plots"
    ofname = os.path.join(odir, "leaf_mass_vs_height_Eucs_vs_EBF_species.pdf")
    fig.savefig(ofname, bbox_inches='tight', pad_inches=0.1)



def plot_leaf_area_vs_height_species(df):
    ...
    df_eucs = df_eucs[['species','h.t', 'a.lf']]
    df_eucs = df_eucs.dropna()
    n_species = np.unique(df_eucs.species)
    n_colours = len(n_species)
    colours = sns.color_palette('hls', n_colors=n_colours)
    ...
    odir = "plots"
    ofname = os.path.join(odir, "leaf_area_vs_height_Eucs_vs_EBF_species.pdf")
    fig.savefig(ofname, bbox_inches='tight', pad_inches=0.1)
...
if __name__ == "__main__":
    ...
    plot_leaf_mass_vs_height_species(df)
    plot_leaf_area_vs_height_species(df)
