...
def compare_within_ebt(df):
    ...
    vcmax_eucs = df_eucs['Vcmax'].values
    vcmax_other = df_other['Vcmax'].values
    data = [vcmax_eucs, vcmax_other]
    ...
    ax.text(1.5, y_max + abs(y_max - y_min)*0.18, stars(p_value),
    ...
    odir = "plots"
    fig.savefig(os.path.join(odir, "vcmax_boxplot_within_EBT.pdf"),
